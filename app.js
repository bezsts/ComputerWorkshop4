...
async function loadMovie() {
    ...
    let moviesResponse = await fetch('https://localhost:5101/api/movies/${movieId}', {
        method: 'get'
    });
...
async function createMovie() {
    try {
        ...
        let moviesResponse = await fetch('https://localhost:5101/api/movies', {
            method: 'post',
            headers: {
        ...
        if (!moviesResponse.ok) {
            console.error('Ошибка запроса:', moviesResponse.statusText);
            return;
        }
...
async function loadPopularMovies() {
    let moviesResponse = await fetch('https://localhost:5101/api/movies/popular', {
        method: 'get'
    });
...
async function ExportAllMovies() {
    let response = await fetch('https://localhost:5101/api/movies/download', {
        method: 'get'
    });
...
async function loadAllUsers() {
    let response = await fetch('https://localhost:5101/api/users', {
        method: 'get'
    });
...
async function loadUser() {
    ...
    let response = await fetch('https://localhost:5101/api/users/${userId}', {
        method: 'get'
    });
...
async function createUser() {
    ...
    for (let i = 0; i < userResponseData.length; i++) {
        list.appendChild(createMovieElement(userResponseData[i]));
    }
}

async function loadWatchedMovies() {
    let response = await fetch('https://localhost:5101/api/users/1/watched-movies', {
        method: 'get'
    });
